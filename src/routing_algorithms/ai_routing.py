...
class AIRouting(BASE_routing):
    ...
    # def feedback(self, drone, id_event, delay, outcome): #note 2
    def feedback(self, drone=None, id_event=None, delay=0, outcome=0):
        if drone is None or id_event is None:
            return  #
    ...
    def relay_selection(self, opt_neighbors, pkd):
        ...
        # available_drones =  [self.drone] + list(map(lambda x: x[1], opt_neighbors)) # ad note 1
        # available_drones = [self.drone] + opt_neighbors  # opt_neighbors là danh sách Drone, không cần lập chỉ mục
        available_drones = [self.drone] + [x[1] if isinstance(x, tuple) else x for x in opt_neighbors]
        action, best_drone = self.get_action(state, available_drones)

        # Store your current action --- you can add several stuff if needed to take a reward later
        if isinstance(pkd, tuple):
            pkd = pkd[0]  # Giả sử Packet nằm trong tuple, cần kiểm tra lại vị trí

...
class MultiAgentEDQN_Routing(BASE_routing):
    ...
    def __init__(self, drone, simulator):
        ...
        # CRITICAL: Ensure MARL system exists and is shared
        # Check if MARL system already exists for this simulator
        if not hasattr(simulator, '_marl_system') or simulator._marl_system is None:
            # First drone - initialize MARL system
            if not hasattr(simulator, 'drones') or len(simulator.drones) == 0:
                print(f"[WARN] No drones yet, deferring MARL system initialization")
                simulator._marl_system = None
                simulator._marl_initialized = False
                ...
                try:
                    ...
                    simulator._marl_n_drones_expected = len(simulator.drones)
                    simulator._marl_initialized = True
                    print(f"[INFO] MARL System initialized successfully")
                    
                    # Get agent for this drone
                    self.marl_system = simulator._marl_system
                    if drone.identifier in self.marl_system['agents']:
                        self.agent = self.marl_system['agents'][drone.identifier]
                    else:
                        print(f"[ERROR] Agent {drone.identifier} not in system (has {len(self.marl_system['agents'])} agents)")
                        self.agent = None
            ...
            # MARL system already exists - just get agent reference
            self.marl_system = simulator._marl_system
            
            # Get agent for this drone
            if drone.identifier in self.marl_system['agents']:
                self.agent = self.marl_system['agents'][drone.identifier]
                print(f"[INFO] Reusing existing MARL system for drone {drone.identifier}")
            else:
                print(f"[ERROR] Agent {drone.identifier} not found in existing MARL system")
                print(f"[INFO] Available agents: {list(self.marl_system['agents'].keys())}")
                self.agent = None
    ...
    def ensure_marl_system_ready(self):
        """
        Đảm bảo MARL system đã sẵn sàng.
        Gọi method này sau khi tất cả drones đã được tạo.
        
        Returns:
            True nếu system ready, False nếu không
        """
        if self.marl_system is not None and self.agent is not None:
            return True
        
        # Try to initialize now
        print(f"[INFO] Attempting to initialize MARL system now...")
        
        if not hasattr(self.simulator, 'drones') or len(self.simulator.drones) == 0:
            print(f"[ERROR] Still no drones in simulator")
            return False
        
        try:
            # Initialize system
            self.simulator._marl_system = self._initialize_marl_system(self.simulator)
            self.simulator._marl_n_drones_expected = len(self.simulator.drones)
            self.simulator._marl_initialized = True
            
            # Update references
            self.marl_system = self.simulator._marl_system
            self.agent = self.marl_system['agents'][self.drone.identifier]
            
            print(f"[INFO] MARL system initialized successfully")
            return True
            
        except Exception as e:
            print(f"[ERROR] Failed to ensure MARL system ready: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def __call__(self, drones, depot, cur_step):
        """
        Make routing object callable for compatibility with simulator
        This is called as: drone.routing(drones, depot, cur_step)
        """
        # This method is called during simulation routing phase
        # For MARL, we don't need to do anything here since relay_selection
        # is called separately when packets need to be routed
        pass
    ...
    def relay_selection(self, packet, optimize_residual_energy=False):
        ...
        """
        try:
            # Step 0: Ensure MARL system ready
            if self.agent is None or self.marl_system is None:
                if not self.ensure_marl_system_ready():
                    # Fallback: random selection
                    print(f"[WARN] MARL system not ready, using random selection")
                    try:

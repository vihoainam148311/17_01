...
class BASE_routing(metaclass=abc.ABCMeta):
    ...
    # === Mô hình bitrate động theo chuẩn Wi-Fi 802.11n phù hợp với wifi dân dụng===
    def get_dynamic_bitrate(self, drone, neighbor):
        distance = util.euclidean_distance(drone.coords, neighbor.coords)
        rssi = self.estimate_rssi(distance)
        return self.map_rssi_to_bitrate(rssi)

    def estimate_rssi(self, distance, tx_power_dbm=17, path_loss_exponent=2.8, reference_distance=1):
        # Mô hình log-distance path loss
        if distance < reference_distance:
            distance = reference_distance
        return tx_power_dbm - 10 * path_loss_exponent * math.log10(distance / reference_distance)

    def map_rssi_to_bitrate(self, rssi_dbm):
        # Mapping RSSI to bitrate theo 802.11n
        if rssi_dbm >= -60:
            return 54000000  # 54 Mbps
        elif rssi_dbm >= -67:
            return 24000000  # 24 Mbps
        elif rssi_dbm >= -75:
            return 12000000  # 12 Mbps
        elif rssi_dbm >= -82:
            return 6000000  # 6 Mbps
        else:
            return 1000000  # 1 Mbps (yếu)
    ...
    def drone_identification(self, drones, cur_step):
        ...
        print(f"[DEBUG] Drone {self.drone.identifier}: Sending hello message at step {cur_step}")

        # Calculate sent/received packet stats
        ...
        my_hello = HelloPacket(self.drone, self.drone.coords, self.drone.residual_energy, self.drone.speed,
                               self.drone.next_target(), self.drone.queuing_delay, self.drone.discount_factor,
                               received_pck, sended_ack)

        # Send to all drones in range
        neighbors_in_range = 0
        for d_drone in drones:
            if d_drone.identifier != self.drone.identifier:
                distance = util.euclidean_distance(self.drone.coords, d_drone.coords)
                if distance <= min(self.drone.communication_range, d_drone.communication_range):
                    neighbors_in_range += 1

        print(f"[DEBUG] Drone {self.drone.identifier}: Broadcasting hello to {neighbors_in_range} drones in range")
    ...
    def send_packets(self, cur_step):
        # FLOW 0 - Debug
        print(
            f"[DEBUG] Drone {self.drone.identifier} send_packets: step={cur_step}, buffer_length={self.drone.buffer_length()}")

        if self.no_transmission or self.drone.buffer_length() == 0:
            if self.drone.buffer_length() == 0:
                print(f"[DEBUG] Drone {self.drone.identifier}: Empty buffer, nothing to send")
            if self.no_transmission:
                print(f"[DEBUG] Drone {self.drone.identifier}: no_transmission flag is True")
            return

        # if the drone is near to the depot, deliver the packets
        depot_distance = util.euclidean_distance(self.simulator.depot.coords, self.drone.coords)
        print(
            f"[DEBUG] Drone {self.drone.identifier}: Distance to depot = {depot_distance}, depot_range = {self.simulator.depot_com_range}")

        if depot_distance <= self.simulator.depot_com_range:
            print(f"[DEBUG] Drone {self.drone.identifier}: Near depot, transferring packets")
        ...
        if cur_step % self.simulator.drone_retransmission_delta == 0:
            print(f"[DEBUG] Drone {self.drone.identifier}: Retransmission step, checking neighbors")

            self.old_neighbors = self.opt_neighbors
            self.opt_neighbors = []

            # Check hello messages
            print(f"[DEBUG] Drone {self.drone.identifier}: Hello messages count = {len(self.hello_messages)}")

            for i in range(self.simulator.n_drones):
                arrival_time = self.drone.neighbor_table[i, 6]
                time_diff = self.simulator.cur_step - arrival_time

                if (time_diff < self.simulator.ExpireTime and arrival_time != 0):
                    neighbor_drone = self.simulator.drones[i]
                    self.opt_neighbors.append(neighbor_drone)
                    print(
                        f"[DEBUG] Drone {self.drone.identifier}: Added neighbor {i}, arrival_time={arrival_time}, time_diff={time_diff}")
                elif arrival_time != 0:
                    print(f"[DEBUG] Drone {self.drone.identifier}: Expired neighbor {i}, time_diff={time_diff}")

            print(f"[DEBUG] Drone {self.drone.identifier}: Total valid neighbors = {len(self.opt_neighbors)}")
            ...
            for pkd in self.drone.all_packets():
                print(
                    f"[DEBUG] Drone {self.drone.identifier}: Processing packet {pkd[0].identifier if hasattr(pkd[0], 'identifier') else 'unknown'}")
                ...
                print(
                    f"[DEBUG] Drone {self.drone.identifier}: Selected neighbor = {best_neighbor.identifier if best_neighbor and hasattr(best_neighbor, 'identifier') else 'None'}")

                # Handle routing hole
                if isinstance(best_neighbor, str):
                    print(f"[DEBUG] Drone {self.drone.identifier}: Routing hole detected: {best_neighbor}")
                    # ... rest of routing hole handling

                elif best_neighbor is not None:
                    print(
                        f"[DEBUG] Drone {self.drone.identifier}: Sending packet to neighbor {best_neighbor.identifier}")
                    ...
                    print(f"[DEBUG] Drone {self.drone.identifier}: No neighbor selected for packet")
    ...
    def geo_neighborhood(self, drones, no_error=False):
        """
        Debug version of geo_neighborhood
        """
        closest_drones = []

        print(f"[DEBUG] Drone {self.drone.identifier}: Checking geo neighborhood among {len(drones)} drones")

        for other_drone in drones:
            if self.drone.identifier != other_drone.identifier:
                drones_distance = util.euclidean_distance(self.drone.coords, other_drone.coords)

                comm_range = min(self.drone.communication_range, other_drone.communication_range)

                if drones_distance <= comm_range:
                    print(
                        f"[DEBUG] Drone {self.drone.identifier}: Found neighbor {other_drone.identifier} at distance {drones_distance:.2f}")
                    ...
                    if self.channel_success(drones_distance, no_error=no_error):
                        ...
                        print(
                            f"[DEBUG] Drone {self.drone.identifier}: Channel success with neighbor {other_drone.identifier}")
                    else:
                        print(
                            f"[DEBUG] Drone {self.drone.identifier}: Channel failed with neighbor {other_drone.identifier}")

        print(f"[DEBUG] Drone {self.drone.identifier}: Total geo neighbors = {len(closest_drones)}")
